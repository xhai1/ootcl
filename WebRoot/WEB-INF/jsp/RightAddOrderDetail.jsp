...
<script src="${pageContext.request.contextPath}/js/achgorder.js"></script>

<script>
	function toPage(){
		var page = $("input[name='number']").val();
		window.location.href="${pageContext.request.contextPath}/addOrder/showAddOrder.action?page="+page+"&cusname=${cusname}&begintime=${begintime}&endtime=${endtime}&cusid=${cusid}&typeid=${typeid}&type=${type}";
	}
	function toExport(root){
		if(root==0){
			if(confirm("只能导出当前页的内容！")){
				window.location.href="${pageContext.request.contextPath}/addOrder/exportAddOrder.action?cusname=${cusname}&begintime=${begintime}&endtime=${endtime}&cusid=${cusid}&typeid=${typeid}&type=${type}&page="+${pageBean.page};
			}
		}else{
...
<div class="grayStripe">补单查询</div>

        <div class = "reportAlign floatright">
            <form name="aChgOrder" action="${pageContext.request.contextPath}/addOrder/showAddOrder.action" method="get">
			                查询条件：
			     <c:choose>
                ...
                <input name="bt" type="button" onmousedown="aChgOrderSearch()" class="report" value="查询" />
                <c:choose>
...
<div class="clearfloat">显示条件：</div>
	...
	<table class="righttable" cellspacing="0px" cellpadding="2px">
		...
		<tbody>
			...
			<!-- 
			<tr class="righttabletr"><td >1</td><td>XXX公司</td><td>2017-10</td><td>2017-10-01</td><td>补单</td><td>彩电</td><td>1710011947021094</td><td>CS1710010827000675</td><td>4960</td><td >东芝彩电</td><td>东芝液晶</td><td>座装</td><td>要求座装</td></tr>
			<tr class="righttabletr2"><td>2</td><td>XXX公司</td><td>2017-10</td><td>2017-10-01</td><td>补单</td><td>彩电</td><td>1710011947021094</td><td>CS1710010827000675</td><td>4960</td><td >东芝彩电</td><td>东芝液晶</td><td>座装</td><td>要求座装</td></tr>
			-->
     <c:if test="${pageBean.list.size() eq 0 }">
			...
			<c:forEach items="${pageBean.list }" var="addOrder" varStatus="ids">
				<tr class="righttabletr2"><td>${ids.index+1 }</td><td>${addOrder.cusname }</td><td>${addOrder.tmonth }</td><td>
				
				
				${fn:substring(addOrder.accdate,0,11)}
				</td><td>${addOrder.acc }</td><td>${addOrder.mode }</td><td>${addOrder.businesstype }</td>
...
<div class="floatleft"><span>记录条数：${pageBean.totalCount }&nbsp;总页数：${pageBean.totalPage }&nbsp;当前页：${pageBean.page }</span></div>
			...
			<div class="floatright">
            	<c:choose>
            		...
            		<c:otherwise>
            			<span class="disabled"><a onclick="aChgOrderToNextPage('${pageContext.request.contextPath}/addOrder/showAddOrder.action?page=1&cusname=${cusname}&begintime=${begintime}&endtime=${endtime}&cusid=${cusid}&typeid=${typeid}&type=${type}')" ><< 首页</a> </span>
            		</c:otherwise>
            	</c:choose>
                
                <c:choose>
                	<c:when test="${pageBean.page>1 }">
                		<span class="disabled"><a  onclick="aChgOrderToNextPage('${pageContext.request.contextPath}/addOrder/showAddOrder.action?page=${pageBean.page-1}&cusname=${cusname}&begintime=${begintime}&endtime=${endtime}&cusid=${cusid}&typeid=${typeid}&type=${type}')" >< 上一页</a> </span>
                	</c:when>
                	<c:otherwise>
                ...
                <c:choose>
                	<c:when test="${pageBean.page<pageBean.totalPage }">
                		<span class="disabled"> <a onclick="aChgOrderToNextPage('${pageContext.request.contextPath}/addOrder/showAddOrder.action?page=${pageBean.page+1}&cusname=${cusname}&begintime=${begintime}&endtime=${endtime}&cusid=${cusid}&typeid=${typeid}&type=${type}')" >下一页 ></a> </span>
                	</c:when>
                	<c:otherwise>
                ...
                <c:choose>
                	...
                	<c:otherwise>
                		<span class="disabled"> <a onclick="aChgOrderToNextPage('${pageContext.request.contextPath}/addOrder/showAddOrder.action?page=${pageBean.totalPage}&cusname=${cusname}&begintime=${begintime}&endtime=${endtime}&cusid=${cusid}&typeid=${typeid}&type=${type}')" >尾页>></a> </span>
                	</c:otherwise>
                </c:choose>
